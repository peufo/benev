<script lang="ts">
	...
	let { pages, sections = [], slug }: Props = $props()
...
{#snippet tree(close?: () => Promise<void>)}
	<ul class="flex flex-col gap-0.5 p-2">
		{#each pages as pageEntry (pageEntry.slug)}
			...
			<li>
				...
				{#if isCurrent && sections.length}
					<ul class="border-soft mt-0.5 ml-4 flex flex-col gap-0.5 border-l pl-1" transition:slide>
