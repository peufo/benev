<script lang="ts">
	import { Surface, TabsIcon } from '$lib/ui'
	...
	let { teams, views, isFullscreen = false, plan = $bindable() }: Props = $props()
...
<Surface title="Planification">
	{#snippet action()}
		...
		<a
			...
			use:tip={{ content: 'Ouvrir en plein écran' }}
		>
			<ExternalLinkIcon size={18} class="opacity-80 {isFullscreen ? 'rotate-180' : ''}" />
		</a>
	{/snippet}
</Surface>
