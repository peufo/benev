...
export const GET = async ({ url, cookies, locals }) => {
	...
	try {
		const { githubUser, createUser } = await githubAuth.validateCallback(code)

		const getExistingUser = async () => {
			if (!githubUser.email) return null
			return prisma.user.findFirst({ where: { email: githubUser.email } })
		}
